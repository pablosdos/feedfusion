...
from product_feed_generator.modules.final_feed_.modifiers.group_tags import (
    apply_grouptag_condition_adjustments,
)
from product_feed_generator.views.helper_OUTDATED import *
from product_feed_generator.modules.final_feed_.modify import _add_custom_calc_fields
...
def create_final_feed_xml_file() -> None:
    ...
    copy: list[dict] = deepcopy(list(selected_products_from_database.values()))
    modified_products: list[dict] = apply_grouptag_condition_adjustments(selected_products_from_database, copy)
    _add_custom_calc_fields(modified_products)
    xml: bytes = dicttoxml(
        modified_products, custom_root="product_final_feed", attr_type=False
    )
